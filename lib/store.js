...
export async function listProducts({ admin = false, withImages = false } = {}) {
  ...
  if (hasSupabase()) {
    ...
    return supabase(`products?select=${columns}&order=${order}`);
  }
...
export async function getProduct(id) {
  if (hasSupabase()) {
    const data = await supabase(`products?select=${PRODUCT_COLUMNS}&id=eq.${Number(id)}&limit=1`);
    return data[0] || null;
  }
  const { row } = await sqlite();
  return row(`SELECT ${PRODUCT_COLUMNS} FROM products WHERE id = ?`, Number(id));
}

export async function listProductImages(ids) {
  const cleanIds = Array.from(new Set(ids.map((id) => Number(id)).filter(Boolean))).slice(0, 80);
  if (cleanIds.length === 0) return [];
  if (hasSupabase()) {
    if (cleanIds.length === 1) {
      return supabase(`products?select=id,image&id=eq.${cleanIds[0]}&limit=1`, { timeoutMs: 18000 });
    }
    return supabase(`products?select=id,image&id=in.(${cleanIds.join(",")})`, { timeoutMs: 18000 });
  }
  const { rows } = await sqlite();
  const result = [];
  for (const id of cleanIds) {
    const productRows = rows("SELECT id, image FROM products WHERE id = ?", id);
