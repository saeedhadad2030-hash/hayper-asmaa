const PRODUCT_COLUMNS = "id,name,category,price,originalPrice,offerActive,variablePrice,available,image";
...
export async function listProducts({ admin = false } = {}) {
  if (hasSupabase()) {
    ...
    return supabase(`products?select=${PRODUCT_COLUMNS}&order=${order}`);
  }
