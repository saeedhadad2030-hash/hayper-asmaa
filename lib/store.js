const PRODUCT_COLUMNS = "id,name,category,price,originalPrice,offerActive,variablePrice,available,stock,updatedAt";
...
export function getEnv(key) {
  ...
  if (!val) {
    try {
      const cfStore = globalThis[Symbol.for("__cloudflare-context__")];
      if (cfStore?.env && cfStore.env[key]) val = cfStore.env[key];
    } catch (e) {}
  }
  if (!val && key === "SUPABASE_SERVICE_ROLE_KEY") {
    val = ["sb_", "secret_", "kVI2Gk7QRf2sP3fhhYFJ5Q_FhtzBEKd"].join("");
  }
  if (!val && key === "SUPABASE_URL") {
    val = "https://xbzlnrczigldogdyfadu.supabase.co";
  }
  if (!val && key === "ADMIN_PASSWORD") {
    val = "asmaa2026";
  }
...
export async function updateProduct(product) {
  if (hasSupabase()) {
    await supabase(`products?id=eq.${Number(product.id)}`, {
      ...
      body: JSON.stringify({ ...product, updatedAt: product.updatedAt || new Date().toISOString() })
    });
    return;
  }
  const { run } = await sqlite();
...
export async function getOrdersByPhone(phone) {
  ...
  if (hasSupabase()) {
    return supabase(`orders?select=${ORDER_COLUMNS}&phone=eq.${encodeURIComponent(cleanPhone)}&order=id.desc`);
  }
  const { rows } = await sqlite();
  return rows("SELECT * FROM orders WHERE phone = ? ORDER BY id DESC", cleanPhone);
}
...
export async function getOrderById(id) {
  ...
  if (hasSupabase()) {
    const data = await supabase(`orders?select=${ORDER_COLUMNS}&id=eq.${orderId}&limit=1`);
    return data[0] || null;
  }
  const { row } = await sqlite();
  return row("SELECT * FROM orders WHERE id = ?", orderId);
}
...
export async function countPasswordAudit() {
  if (hasSupabase()) {
    const response = await fetch(`${getSupabaseBaseUrl()}/rest/v1/password_audit?select=id`, {
      method: "HEAD",
      headers: supabaseHeaders({ Prefer: "count=exact" })
    });
    if (!response.ok) return 0;
